...
`./install-zkcode.command` 对每个外部步骤设置了有限重试和最长等待：管理员密码确认
最多等待 5 分钟，Apple Command Line Tools 最多等待 30 分钟，Homebrew/Rust/单个 Homebrew 公式最多 20～30 分钟，
项目依赖安装和首次构建最多 45 分钟，服务启动最多 3 分钟。超时后会终止当前步骤、
保留已经正确安装的内容并返回非零状态；网络恢复后可安全地重新执行同一命令。
...
当前门禁要求 Rust 1.97+、Node.js 22、npm 10，以及 Python 3.11 或 3.12。
...
重新执行 `./scripts/setup-macos.sh`；前端必须使用 `npm ci`，Python 必须使用
...
```bash
python-service/.venv/bin/python -m playwright install chromium
```
...
启动或在崩溃后自动无限重启。如果 PID 文件存在但服务异常，先运行 `./stop.sh` 精确
...
并检查后端日志。正常 socket 权限是 `0600`。重新执行安装脚本可重建项目 `.venv`，
但不会改写已有 `.env`。

### 后端健康检查超时

`start.sh` 最多等待后端 90 秒。检查 `.runtime/backend.log` 中最早的错误；常见原因
包括 SQLite 路径不可写、MCP 注册表路径错误、`.env` shell 语法错误或 Python
sidecar 启动失败。

## 页面能打开但不能对话

1. 确认 `./scripts/doctor.sh` 同时显示前端和后端正在运行。
2. 检查 `.env` 至少有一个非空 `LLM_PROVIDER_*_API_KEY`，且模型清单包含
   `ZK_DEFAULT_MODEL`。
3. 执行 `./stop.sh`、`./start.sh`，确保配置已重新加载。
...
请附上 zkcode 版本、macOS/芯片、`doctor.sh` 输出、最小复现步骤和已脱敏日志。
