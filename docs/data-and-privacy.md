...
| 仓库根 `.zk/data.db` | Project、Session、消息、Run、Task、证据、产物元数据和审计记录 | 视为私密对话与项目数据 |
...
访问令牌文件和 sidecar socket 会限制为当前 macOS 用户访问。它们不能阻止同一
用户权限下的恶意进程读取数据，因此 zkcode 不是操作系统级沙箱。
